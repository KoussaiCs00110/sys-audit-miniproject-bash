...
#  Generates .txt and .json reports from HW[] and SW[] data.
# ============================================================

# ── Filename helpers 
_report_base() {
    local type="$1"   # short | full
    local ts
    ts=$(date '+%Y%m%d_%H%M%S')
    echo "${REPORT_DIR}/audit_${type}_$(hostname -s)_${ts}"
}

LAST_REPORT_TXT=""   # set after generation; used by email/remote modules
LAST_REPORT_HTML=""  # used by email module for the HTML body
